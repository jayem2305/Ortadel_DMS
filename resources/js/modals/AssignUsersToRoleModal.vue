<template>
  ...
  >
    <div class="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
      ...
      <!-- Body -->
      <div class="p-6">
        <div v-if="role" class="mb-4 p-3 bg-purple-50 rounded-lg">
          <p class="text-sm text-gray-600">Role:</p>
          <p class="font-semibold text-gray-900">{{ role.name }}</p>
          <p class="text-xs text-gray-600">{{ role.type }}</p>
          ...
          <div v-else class="divide-y divide-gray-200">
            ...
            >
              ...
              <div class="ml-3 flex-1">
                ...
                <p v-if="user.role" class="text-xs text-gray-500 mt-1">
                  Current Role: <span class="font-medium">{{ user.role.name }}</span>
                </p>
              </div>
            </label>
          </div>
      ...
      <div class="flex items-center justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50">
        <button
          @click="handleClose"
          :disabled="processing"
          class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 disabled:opacity-50"
        >
          Cancel
        </button>
        <button
          @click="handleAssign"
          :disabled="processing"
          class="px-4 py-2 text-sm font-medium text-white bg-purple-600 rounded-lg hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {{ processing ? 'Assigning...' : 'Assign Users to Role' }}
        </button>
      </div>
    </div>
  </div>
...
const handleAssign = async () => {
  ...
  try {
    ...
    successMessage.value = `Successfully assigned ${selectedUserIds.value.length} user(s) to role!`
    ...
    errorMessage.value = err.response?.data?.message || 'Failed to assign users to role'
  } finally {
  ...
  if (newVal) {
    ...
    if (props.role?.id) {
      selectedUserIds.value = users.value
        .filter(u => u.role_id === props.role.id)
        .map(u => u.id)
    }
  } else {
    resetForm()
