...
const submit = async () => {
  ...
  loading.value = true
  try {
    await axios.post('http://127.0.0.1:8000/api/keywords', form.value)
...
<template>
  ...
  >
    ...
    >
      <!-- Header -->
      <div
        class="bg-gradient-to-r from-blue-600 to-blue-500 text-white px-6 py-3 flex justify-between items-center"
      >
        <h2 class="text-lg font-semibold">Add Keyword</h2>
        <button
          @click="$emit('close')"
      ...
      <form @submit.prevent="submit" class="p-6 space-y-4">
        ...
        <div class="flex justify-end gap-2 pt-4 border-t">
          ...
          <button
            type="submit"
            :disabled="loading"
            class="px-4 py-2 rounded-lg text-sm bg-blue-600 text-white hover:bg-blue-700 transition disabled:opacity-50"
          >
...
<style scoped>
/* Optional subtle fade-in animation for modal */
@keyframes fadeIn {
  from {
    opacity: 0;
